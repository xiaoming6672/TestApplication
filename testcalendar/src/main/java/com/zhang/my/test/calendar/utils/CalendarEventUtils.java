package com.zhang.my.test.calendar.utils;
...
/**
 * @author ZhangXiaoMing 2020-07-13 14:12 星期一
 */
public class CalendarEventUtils {
    ...
    public static final String CALENDARS_NAME = "Test";
    public static final String CALENDARS_ACCOUNT_NAME = "account_test";
    public static final String CALENDARS_ACCOUNT_TYPE = "account_type";
    public static final String CALENDARS_DISPLAY_NAME = "display_name";

    /**
     * 检查是否已经添加了日历账户，如果没有添加先添加一个日历账户再查询 获取账户成功返回账户id，否则返回-1
     */
     ...
     * @param reminderTime 事件开始事件
     * @param previousTime 事件提前多少事件提醒
     */
    public static void addCalendarEvent(Context context, String title, String description,
                                        long reminderTime, int previousTime) {
        if (context == null) {
            return;
        }
        int calId = checkAndAddCalendarAccount(context); //获取日历账户的id
        if (calId < 0) { //获取账户id失败直接返回，添加日历事件失败
            Toast.makeText(context, "获取日历账户id失败", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (newEvent == null) { //添加日历事件失败直接返回
            Toast.makeText(context, "日历事件添加失败", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        values.put(CalendarContract.Reminders.MINUTES, previousTime * 24 * 60);// 提前previousDate天有提醒
        values.put(CalendarContract.Reminders.METHOD, CalendarContract.Reminders.METHOD_ALERT);
        Uri uri = context.getContentResolver().insert(Uri.parse(CALENDER_REMINDER_URL), values);
        if (uri == null) { //添加事件提醒失败直接返回
            Toast.makeText(context, "事件提醒添加失败", Toast.LENGTH_SHORT).show();
            return;
        }

        Toast.makeText(context, "日历事件添加成功", Toast.LENGTH_SHORT).show();
    }
